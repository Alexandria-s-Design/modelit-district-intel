# Banning Unified School District - Entry Strategy

**Last Updated**: 2026-02-12
**Pipeline Stage**: 1 - Identified
**District Type**: Medium sub-urban K-12 unified district (4,316 students)

---

## Primary Contact

| Field | Detail |
|-------|--------|
| **Name** | Marcia Cole-Fijabi |
| **Title** | Director, Instructional Services |
| **Email** | mcolefijabi@banning.k12.ca.us *(inferred — verify)* |
| **Phone** | (951) 922-0200 |
| **Why this person** | Operationally owns curriculum evaluation and program selection under Asst. Supt. Causey-Bush. Fastest path to instructional adoption. Start here. |

## Secondary Contact

| Field | Detail |
|-------|--------|
| **Name** | Dr. Tonia Causey-Bush |
| **Title** | Assistant Superintendent, Educational Services |
| **Email** | tcausey-bush@banning.k12.ca.us *(inferred — verify)* |
| **Phone** | (951) 922-0200 |
| **Why this person** | Executive approver for all curriculum and instruction decisions. Engage after Cole-Fijabi shows interest. |

---

## The Hook

> "Banning USD outperformed both Riverside County and the entire state in math growth for 2024-25 — +7.75% in 11th grade and +5.96% in 7th grade. Your district has proven that interventions work and students can improve. But only 15.3% overall are meeting math standards — 19 points below state average. ModelIt extends that success by adding the computational layer your students need. Students BUILD models of real systems — not just click through simulations. They use variables, equations, and graphs to solve problems they care about. And with 75% Hispanic students, ModelIt's fully bilingual interface means every student can build models in English or Spanish — making computational thinking accessible to all."

---

## Budget Pathway

| Source | Est. Available | Timeline | Likelihood | Notes |
|--------|---------------|----------|-----------|-------|
| **LCFF Supplemental/Concentration** | $100-300K | July 2026 | HIGH | Very high unduplicated count (74.9% Hispanic + high ED) |
| **Title I** | $70-150K | Current year or July 2026 | HIGH | High ED qualifies; math intervention |
| **Title III** | $30-80K | July 2026 | MEDIUM | Bilingual ModelIt for EL students |
| **Title IV-A (SSAE)** | $25-50K | July 2026 | MEDIUM | STEM allocation |
| **EEBG** | $10-20K | Current year | MEDIUM | PD component |

**Primary Pathway**: LCFF Supplemental/Concentration + Title I co-funding for grades 5-12 math/science intervention. Title III as secondary for bilingual feature.

---

## Champions to Cultivate

| # | Name | Title | Leverage Point |
|---|------|-------|---------------|
| 1 | Marcia Cole-Fijabi | Director, Instructional Services | Operational curriculum buyer; fastest path to adoption |
| 2 | Dr. Tonia Causey-Bush | Asst. Supt., Educational Services | Executive approver; owns all K-12 curriculum decisions |
| 3 | Steve Barriga | Innovation Technology Officer | Technology platform gatekeeper; must approve new digital tools |
| 4 | Dr. Cami Thomas | Principal, Nicolet Middle School | Top pilot site ("School to Watch"); can authorize school-level pilot |
| 5 | Victor Herrera | Asst. Supt., Business Services | Budget authority; identifies Title I + LCFF Supplemental funding |
| 6 | Art Paz | Principal, Banning High School | CTE + A-G completion pilot; grades 9-12 implementation site |
| 7 | Terrence Davis | Superintendent | Final budget authority; approach after Causey-Bush alignment |
| 8 | Mayra Anguiano / Lucia Martinez-Lara | Board Trustees | Equity + bilingual STEM champions on board; 2024 electees |

---

## Decision Process

In a K-12 district:
1. **Instructional Services Director evaluates** → Marcia Cole-Fijabi reviews ModelIt for curriculum fit and NGSS alignment
2. **Asst. Superintendent champions** → Dr. Tonia Causey-Bush champions to Superintendent for budget support
3. **Technology Officer approves platform** → Steve Barriga approves ModelIt for district device ecosystem
4. **Superintendent approves** → Terrence Davis signs off on tool and budget
5. **CBO confirms funding** → Victor Herrera + Eddie Ramos identify Title I, LCFF Supplemental, or Title III funds
6. **Board approval** → Only if purchase exceeds district threshold; route through Edith Valencia (evalencia@banning.k12.ca.us)
7. **Implementation** → Nicolet Middle ("School to Watch") first, then Banning High (9-12); expand to Florida Street Discovery Center (K-8)

**No formal RFP** expected for a pilot.

---

## 5-Phase Approach

### Phase 1: Introduction (Mar-Apr 2026)

| Action | Detail | Timeline |
|--------|--------|----------|
| Email to Marcia Cole-Fijabi | mcolefijabi@banning.k12.ca.us (est.) — position ModelIt as NGSS computational modeling, math reasoning | Mar 2026 |
| Parallel email to Steve Barriga | sbarriga@banning.k12.ca.us (est.) — Chromebook + Google Workspace fit, NGSS alignment | Mar 2026 |
| Follow-up call | (951) 922-0200 → ask for Instructional Services; discuss pilot opportunity | Mar-Apr 2026 |
| Email Dr. Causey-Bush | tcausey-bush@banning.k12.ca.us (est.) — after Cole-Fijabi engagement | Apr 2026 |

### Phase 2: Demo & Discovery (Mar-Apr 2026)

| Action | Detail | Timeline |
|--------|--------|----------|
| Site visit | Visit Banning High, meet principal | Mar-Apr 2026 |
| Demo for leadership | Show math models, NGSS alignment | Same visit |
| Bilingual demo | Show Spanish-language modeling for EL teachers | Same visit |

### Phase 3: Pilot Proposal (Apr-May 2026)

| Action | Detail | Timeline |
|--------|--------|----------|
| Written proposal | 1-page pilot targeting grades 5-12, NGSS-aligned | Apr 2026 |
| Budget proposal | $15-25K for high school + middle school | May 2026 |
| Title III integration | Position within bilingual funding for EL students | May 2026 |

### Phase 4: Pilot Implementation (Aug-Dec 2026)

| Action | Detail | Timeline |
|--------|--------|----------|
| Teacher training | Half-day PD for participating teachers | Aug 2026 |
| Pilot launch | ModelIt at Banning High (9-12) + middle schools | Sep 2026 |
| Student models | Students build computational models across science topics | Sep-Dec 2026 |
| Mid-pilot review | Usage data and teacher feedback | Nov 2026 |

### Phase 5: Evaluation & Expansion (Jan-Mar 2027)

| Action | Detail | Timeline |
|--------|--------|----------|
| Pilot results report | Math engagement, student usage, teacher satisfaction | Jan 2027 |
| K-12 expansion | Add remaining grades and schools | Feb 2027 |
| LCAP integration | Include ModelIt in 2027-28 LCAP | Mar-May 2027 |

---

## Risks & Mitigation

| Risk | Likelihood | Impact | Mitigation |
|------|-----------|--------|-----------|
| Cole-Fijabi doesn't respond | Medium | High | Call (951) 922-0200 → ask for Instructional Services. Escalate to Dr. Causey-Bush directly. |
| Budget too tight (below median per-pupil) | High | Medium | Position within Title I or LCFF Supplemental — earmarked funds. $15-25K pilot is minimal. |
| 15.3% math = "we need basic intervention" | Medium | Medium | Frame ModelIt as applied math, not another worksheet program. Students build models of real systems. |
| Science curriculum adoption unknown | Medium | Medium | Position ModelIt as NGSS bridge regardless of core curriculum. Complements, doesn't replace. |
| Small district = small deal | Certain | Low | $15-25K deal. Value is in Riverside County case study + RCOE connection for neighboring districts. |
| Multiple priorities (graduation focus) | Low | District emphasis on college/career may compete with STEM funding |
| Contact information incomplete | Medium | High | Need to verify district website for staff directory |

---

## Competitive Positioning

| Current Tool/Program | Our Message |
|---------------------|-------------|
| [Free simulations] | "Free sims let students click through pre-built activities. ModelIt lets them BUILD models — that's the mathematical and computational thinking NGSS demands." |
| [Textbook curriculum] | "Core curriculum delivers content. ModelIt adds the computational thinking layer — students build systems, observe them, refine them." |
| [Traditional math intervention] | "Worksheets teach rules. ModelIt teaches mathematical reasoning through applied modeling — students USE math to solve problems." |
| [CTE pathways] | "ModelIt enhances CTE by adding computational modeling skills. Biotech, health science, environmental pathways all use systems thinking." |

---

## Success Metrics for Pilot

| Metric | Target | Measurement |
|--------|--------|-------------|
| Student usage | 80%+ of pilot students complete ≥3 models | Platform analytics |
| Teacher satisfaction | 4+/5 rating | Post-pilot survey |
| NGSS alignment | Models mapped to ≥3 NGSS PEs | Curriculum review |
| Bilingual usage | ≥30% of models built in Spanish | Platform analytics |
| Math impact | Teacher-reported math engagement increase | Survey |
| CTE enhancement | ≥1 CTE pathway uses ModelIt | Teacher report |
| Reorder intent | Dr. Causey-Bush commits to Year 2 | Direct conversation |

---

## Key Dates Calendar

| Date | Event | Action |
|------|-------|--------|
| Mar 2026 | Email window | Email Cole-Fijabi + Barriga |
| Apr 2026 | Site visit | Nicolet Middle + Banning High demo |
| Apr 2026 | LCAP input period | Request presentation |
| Jun 2026 | LCAP/budget approval | Pilot proposal ready |
| Aug 2026 | School year starts | Teacher training |
| Sep 2026 | Pilot launch | Implementation begins |
